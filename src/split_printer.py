...
def get_results_root(args):
    return os.path.join(args.root, args.dataset, args.model, args.task)
...
def get_rsum(dataf):
    return (
        dataf["t2i_recalls_at_1"]
        + dataf["t2i_recalls_at_5"]
        + dataf["t2i_recalls_at_10"]
    )
...
def main(args):
    ...
    answers = []
    for perturbation_file in ordered_perturbation_files:
        ans = ""
        perturbation_file = f"{perturbation_file}-results.pkl"
        ...
        if os.path.exists(perturbation_path):
            ...
            dataset_no_perturbation["t2i_rsum"] = get_rsum(
                dataf=dataset_no_perturbation
            )
            dataf_perturbation["t2i_rsum"] = get_rsum(dataf=dataf_perturbation)
            # print(dataf_perturbation['t2i_rsum'])

            dataf_perturbation["original_query"] = dataset_no_perturbation[
                "t2i_queries"
            ]
            ...
            merged_df["rsum_diff"] = (
                merged_df["t2i_rsum_none"] - merged_df["t2i_rsum_perturbation"]
            )
            print(dataf_perturbation)
            ...
            ans = f"{round(df_rsum_decreased.shape[0]*100/len_df, 2)}\t{round(df_rsum_increased.shape[0]*100/len_df, 2)}\t{round(df_rsum_unchanged.shape[0]*100/len_df, 2)}\t{perturbation_file[:15]}"
            ...
            file_rsum_decreased = os.path.join(
                results_root, f"{perturbation_name}-rsum-decreased.pkl"
            )
            file_rsum_increased = os.path.join(
                results_root, f"{perturbation_name}-rsum-increased.pkl"
            )
            file_rsum_unchanged = os.path.join(
                results_root, f"{perturbation_name}-rsum-unchanged.pkl"
            )
            ...
            answers.append(ans)
            # answers.append('test')
            # print(f'appended {ans} to the list\n now anwers look like this: {answers}')
            # print('ans: ', answers)
        else:
            ans = f"0.0\t0.0\t0.0\t{perturbation_file}"
            print(f"ans\n")
            answers.append(ans)
            # print(f'appended {ans} to the list\n now anwers look like this: {ans}')
    for el in answers:
        print(el)
