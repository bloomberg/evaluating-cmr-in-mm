...
AVAILABLE_MODELS = "nvidia/groupvit-gcc-yfcc"
...
class GroupViT(nn.Module):
    def __init__(self, config: munch.Munch) -> None:
        ...
        Args:
            self (src.models.encoders.groupvit.GroupViT): instance of the class
            config (munch.Munch): configuration file (depends on dataset and model)
        """

        super(GroupViT, self).__init__()
    ...
    def encode(
        self, x: Union[List[str], List[PIL.Image.Image]], convert_to_tensor: bool
    ) -> np.ndarray:
        if isinstance(x[0], PIL.Image.Image):
            inputs = self.processor(
                images=x, text=["dummy_text"], return_tensors="pt", padding=True
            )
        elif isinstance(x[0], str):
            dummy_img = Image.new("RGB", (640, 480)).convert("RGB")
            inputs = self.processor(
                images=[dummy_img], text=x, return_tensors="pt", padding=True
            )
        else:
