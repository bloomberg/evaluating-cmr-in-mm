...
AVAILABLE_MODELS = "BAAI/AltCLIP"


class AltCLIP(nn.Module):
    def __init__(self, config: munch.Munch) -> None:
        """Init function

        Args:
            self (src.models.encoders.altclip.AltCLIP): instance of the class
            config (munch.Munch): configuration file (depends on dataset and model)
        """

        super(AltCLIP, self).__init__()
        ...
        self.backbone = AltCLIPModel.from_pretrained(self.model_name)

    def encode(
        self, x: Union[List[str], List[PIL.Image.Image]], convert_to_tensor=False
    ) -> np.ndarray:
    ...
    def compute_image_embeddings(
        self, chunk_size=100, compute_from_scratch=False
        ...
        if os.path.exists(emb_path) and not compute_from_scratch:
            print("Image embeddings are already precomputed")
