...
from src.utils.dataset_preprocessing import (
    get_img_filenames_full_path,
    get_img_filenames,
    divide_chunks,
    get_precomputed_embeddings_path,
    load_filenames_embs_from_pkl,
    get_emb_file_path,
    dump_filenames_embs_to_pkl,
)
import requests
import torch
import datetime
...
AVAILABLE_MODELS = "BridgeTower/bridgetower-large-itm-mlm-itc"


class BridgeTower(nn.Module):
    def __init__(self, config: munch.Munch) -> None:
        """Init function

        Args:
            self (src.models.encoders.bridgetower.BridgeTower): instance of the class
        ...
        self.backbone = BridgeTowerForContrastiveLearning.from_pretrained(
            self.model_name
        )

    def encode(self, x: Union[List[str], List[PIL.Image.Image]]) -> np.ndarray:
        batch = []
        if isinstance(x[0], PIL.Image.Image):
            for image in x:
                inputs = self.processor(
                    images=image, text="dummy_text", return_tensors="pt", padding=True
                )
                outputs = self.backbone(**inputs)
                out_emb = outputs["image_embeds"]
                out_emb = out_emb.detach().numpy()
                batch.append(out_emb)

        elif isinstance(x[0], str):
            dummy_img = Image.new("RGB", (640, 480)).convert("RGB")
            for txt in x:
                inputs = self.processor(
                    images=dummy_img, text=txt, return_tensors="pt", padding=True
                )
                outputs = self.backbone(**inputs)
    ...
    def compute_caption_embeddings(
        self, ds_split: Type[Dataset], chunk_size=1000, compute_from_scratch=False
        ...
        if os.path.exists(emb_path) and not compute_from_scratch:
            ...
            caption_ids, capts, capt_emb = load_filenames_embs_from_pkl(
                emb_file_path=emb_path
            )
            ...
            capts = [
                ds_split.captions[caption_id]["raw"][: self.config.model.max_seq_length]
    ...
    def compute_image_embeddings(
        self, chunk_size=1000, compute_from_scratch=False
        ...
        if os.path.exists(emb_path) and not compute_from_scratch:
            ...
            img_root = os.path.join(
                self.config.dataset.root, self.config.dataset.img_folder
            )
            ...
            for idx, chunk in enumerate(imgs_full_paths_chunks):
                ct = datetime.datetime.now()
                print(
                    f"{ct} Computing embeddings progress: {idx}/{len(imgs_full_paths_chunks)}"
                )
                self.config.logging.info(
